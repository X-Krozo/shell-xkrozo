...
FocusScope {
    ...
    anchors.fill: parent
    anchors.leftMargin: root.geometry.insetLeft(root.borderThickness)
    anchors.topMargin: root.geometry.insetTop(root.borderThickness)
    anchors.rightMargin: root.borderThickness
    anchors.bottomMargin: root.geometry.insetBottom(root.borderThickness)
    visible: offsetScale < 1
    opacity: 1 - offsetScale
    transform: Translate {
        id: slideTransform
        y: -root.height * root.offsetScale
    }

    CustomMouseArea {
        anchors.fill: parent
        onPressed: root.screenState.mediaFullscreen = false
    }
    ...
    Item {
        ...
        Item {
            ...
            CustomMouseArea {
                anchors.fill: parent
                onPressed: {
                    // Swallow clicks inside the content area so the backdrop close only triggers on margins
                }
            }
